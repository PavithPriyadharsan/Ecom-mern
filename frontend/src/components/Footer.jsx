    ...
    <footer className="flexCenter pb-24 pt-20">
      <div className="max_padd_container flex w-full flex-col gap-14">
        <div className="flex flex-col items-start justify-center gap-[10%] md:flex-row">
          <Link to="/" className="mb-10 bold-20">Shoppy</Link>
            ...
            <FooterColumn>
              <ul className="flex gap-4">
                {SOCIALS.links.map((link)=>(
                  <Link to="/" key={link}><img src={link} alt="socials" height={22} width={22} /></Link>
                ))}
              </ul>
            </FooterColumn>
          </div>
      ...
      <p className="text-center regular-14 text-gray-30">2024 Shoppy | All rights reserved.</p>
      </div>
    </footer>
  )
}
